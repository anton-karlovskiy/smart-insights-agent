...
- `Benchmark`: `website_count`, `mean_opt_in_rate`, `median_opt_in_rate`, `min_opt_in_rate`, `max_opt_in_rate`, `canonical_industry_segment`, `top_performer_ids: list[int]`, `low_confidence: bool` (the thin-segment guard, §4.4).
...
- One call per **clean** row only — anomalous rows already have `insight = None` and are skipped. Sequential is fine; set `max_output_tokens` with headroom for gpt-5's reasoning tokens, which are drawn from the same budget (the implementation uses 8192 — 2048 truncated real responses mid-JSON). Use `client.responses.parse()` with the `Insight` model as `text_format` so responses are schema-enforced by the API (strict JSON schema), not just parsed hopefully. Read `response.output_parsed`, and treat a refusal or a `None` parse as a validation failure, not a crash.
...
- `out/insights.json`: array of `{id, website_url, canonical_industry_segment, opt_in_rate, cleaned_setup_notes, impossible_metric_anomaly, edge_case_anomaly, benchmark, top_performers, insight, status, status_reason}` for every input row — the row carries everything the grounding check reads (§4.6), so `evaluate` can re-verify from this file alone, offline. Status is `ok`, `needs_review`, or `llm_skipped` (in `--no-llm` mode); anomalous rows are `ok` — they were processed correctly, and their anomaly fields tell the story. `status_reason` carries the `needs_review` explanation (the validation or API error) and is `null` otherwise.
